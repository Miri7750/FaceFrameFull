...
//       </Typography>
//       <ImageUploader />
//       <Masonry  columns={{ xs: 1, sm: 2, md: 3 }}  spacing={2} sx={{ mt: 4 }}>
//         {images.map((img) => (
//           <Card
//             key={img.id}
//             sx={{
...
export default function Images() {
  ...
  const handleEditSave = async () => {
    try {
      setLoadingImageId(editDialog.imageId);
      const formData = new FormData();
      formData.append('file', editFile || new Blob());
      formData.append('image_name', editDialog.imageName);
      await axios.put(`/images/${editDialog.imageId}`, formData);
  ...
  return (
    <Container sx={{ py: 6 }}>
        ...
        {images.map((img) => (
          <Card
            ...
            sx={{
              position: 'relative',
              transition: 'transform 0.3s ease, box-shadow 0.3s ease',
              opacity: loadingImageId === img.id ? 0.5 : 1,
              '&:hover': {
                transform: 'scale(1.02)',
                boxShadow: '0 0 10px rgba(0,229,255,0.6)',
              },
      ...
      <Dialog open={editDialog.open} onClose={() => setEditDialog({ ...editDialog, open: false })}>
        ...
        <DialogActions>
          <Button onClick={() => setEditDialog({ ...editDialog, open: false })}>Cancel</Button>
          <Button onClick={handleEditSave} disabled={loadingImageId === editDialog.imageId}>
            {loadingImageId === editDialog.imageId ? <CircularProgress size={20} /> : 'Save'}
          </Button>
        </DialogActions>
      </Dialog>
    </Container>
